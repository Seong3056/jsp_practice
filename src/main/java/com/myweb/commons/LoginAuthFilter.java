 ...
 */
//@WebFilter("*.board") //.board로 끝나는 모든 요청에 반등
@WebFilter(
...
public class LoginAuthFilter extends HttpFilter implements Filter {
	...
	public void destroy() {
		...
		//보통 초기화시 생성했던 자원들을 종료하는 기능으로 사용한다.
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		/*
		 ...
		 - request객체와 response 객체를 넘겨주기 때문에 이를가지고 요청과 응답을 조작할 수 있다.
		...
		//유효성 문제가 없다면 필터를 통과하게 한다.
		//통과한 요청은 서블릿 클래스로 진입하게 된다.
	...
	public void init(FilterConfig fConfig) throws ServletException {
		...
		//필터에서 처리 시 필요한 객체등을 초기화(JDBC 관련 설정) 하는데 사용한다.
