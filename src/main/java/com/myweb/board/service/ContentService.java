...
//		BoardDAO dao = BoradDAO.getInstance();
//		BoardVo vo = dao.contentBoard(bId);
        ...
        */
		...
		boolean flag = false;
		for(int i =0 ; i<cookies.length;i++) {//요청한 사용자의 모든 
			if(cookies[i].getName().equals(request.getParameter("bId"))) {
				flag = true;
			}
				}
		if(!flag) {//해당 게시글의 쿠키가 없을경우 쿠키를 생성하고 조회수를 올림.
